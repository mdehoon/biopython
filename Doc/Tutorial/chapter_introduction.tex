...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item For \verb+biopython.PDB+: \cite[Hamelryck and Manderick, 2003]{hamelryck2003a};
    \item For \verb+biopython.Cluster+: \cite[De Hoon \textit{et al.}, 2004]{dehoon2004};
    \item For \verb+biopython.Graphics.GenomeDiagram+: \cite[Pritchard \textit{et al.}, 2006]{pritchard2006};
    \item For \verb+biopython.Phylo+ and \verb+biopython.Phylo.PAML+: \cite[Talevich \textit{et al.}, 2012]{talevich2012};
...
...
\end{minted}
  If the ``\verb|import biopython|'' line fails, Biopython is not installed.
  ...
  \item \emph{What file formats do} \verb|biopython.SeqIO| \emph{and} \verb|biopython.AlignIO| \emph{read and write?} \\
  Check the built in docstrings (\texttt{from biopython import SeqIO}, then \texttt{help(SeqIO)}), or see \url{http://biopython.org/wiki/SeqIO} and \url{http://biopython.org/wiki/AlignIO} on the wiki for the latest listing.

  \item \emph{Why won't the } \verb|biopython.SeqIO| \emph{and} \verb|biopython.AlignIO| \emph{functions} \verb|parse|\emph{,} \verb|read| \emph{and} \verb|write| \emph{take filenames? They insist on handles!} \\
  ...
  \item \emph{Why won't the } \verb|biopython.SeqIO.write()| \emph{and} \verb|biopython.AlignIO.write()| \emph{functions accept a single record or alignment? They insist on a list or iterator!} \\
  ...
  \item \emph{Why doesn't} \verb|biopython.Blast| \emph{work with the latest plain text NCBI blast output?} \\
  ...
  \item \emph{Why has my script using} \verb|biopython.Entrez.efetch()| \emph{stopped working?} \\
  ...
  \item \emph{Why doesn't} \verb|biopython.Blast.NCBIWWW.qblast()| \emph{give the same results as the NCBI BLAST website?} \\
  ...
  \item \emph{Why doesn't} \verb|biopython.SeqIO.index_db()| \emph{work? The module imports fine but there is no \texttt{index\_db} function!} \\
  You need Biopython 1.57 or later (and a Python with SQLite3 support).

  \item \emph{Where is the} \verb|MultipleSeqAlignment| \emph{object? The} \verb|biopython.Align| \emph{module imports fine but this class isn't there!} \\
  You need Biopython 1.54 or later. Alternatively, the older \verb|biopython.Align.Generic.Alignment| class supports some of its functionality, but using this is now discouraged.
  ...
  One thing to know is that we put code in \verb|__init__.py| files. If you are not used to looking for code in this file this can be confusing. The reason we do this is to make the imports easier for users. For instance, instead of having to do a ``repetitive'' import like \verb|from biopython.GenBank import GenBank|, you can just use \verb|from biopython import GenBank|.

  \item \emph{Why doesn't} \verb|biopython.Fasta| \emph{work?} \\
  We deprecated the \verb|biopython.Fasta| module in Biopython 1.51 (August 2009) and removed it in Biopython 1.55 (August 2010). There is a brief example showing how to convert old code to use \verb|biopython.SeqIO| instead in the \href{https://github.com/biopython/biopython/blob/master/DEPRECATED.rst}{DEPRECATED.rst} file.
