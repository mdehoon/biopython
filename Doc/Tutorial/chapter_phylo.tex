\chapter{Phylogenetics with biopython.Phylo}
\label{chapter:phylo}

The biopython.Phylo module was introduced in Biopython 1.54. Following the lead of SeqIO and AlignIO,
...
The \verb+biopython.Phylo+ module is described in an open-access journal article \cite[Talevich
...
object called Phylogeny, from the \verb+biopython.Phylo.PhyloXML+ module.
...
if you use these attributes in biopython.Phylo, you will only be able to save the values in
...
The rest of this chapter covers the core functionality of biopython.Phylo in greater detail. For more
examples of using biopython.Phylo, see the cookbook page on Biopython.org:
...
While \verb+biopython.Phylo+ doesn't infer trees from alignments itself, there are third-party
programs available that do. These are supported through the module
\texttt{biopython.Phylo.Applications}, using the same general framework as
\texttt{biopython.Emboss.Applications}, \texttt{biopython.Align.Applications} and others.
...
are already available through the EMBOSS wrappers in \texttt{biopython.Emboss.Applications} if
...
The \verb+biopython.Phylo+ is under active development. Here are some features we might add in future
releases:
...
\begin{description}
  \item[New methods]
    ...
    them to \verb+biopython.Phylo+:

    \url{http://biopython.org/wiki/Phylo_cookbook}

  \item[biopython.Nexus port]
    ...
    existing biopython.Nexus module to the new classes used by biopython.Phylo.

    Currently, biopython.Nexus contains some useful features that have not yet been ported to
    \verb+biopython.Phylo+ classes --- notably, calculating a consensus tree. If you find some functionality
    lacking in \verb+biopython.Phylo+, try poking through \verb+biopython.Nexus+ to see if it's there instead.
