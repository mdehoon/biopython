...
all_chr_info = {"I": chr1_info, "II": chr2_info, "III": chr3_info, "IV": chr4_info}
...
class OrganismSubAnnotationsTest(unittest.TestCase):
    ...
    def check_simple_tRNA(self, filename, use_seqfeatures=False):
        ...
        # fmt: on
        entries = [
            ("Chr I", "NC_003070", 30432563, f1, colors.red),
        ...
        for name, acc, length, features, color in entries:
            if False:
                ...
                features = [
                    (start, end, strand, label, color)  # noqa: C416
                    for (start, end, strand, label) in features
                ]
            ...
            cytobands.append(
                (length - 1000000, length, 0, "Last 1 Mbp", colors.black, colors.brown)
            )
            ...
            if name == "Chr III":
                ...
                cytobands.append(
                    (9500000, 11000000, +1, "Forward", colors.red, colors.yellow)
                )
...
class ChromosomeCountTest(unittest.TestCase):
    ...
    def test_get_segment_info(self):
        ...
        self.count_display.add_count(self.names[test_count_num], test_count_value)
        self.count_display.add_label(self.names[test_label_num], test_label_value)
