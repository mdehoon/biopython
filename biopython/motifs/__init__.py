...
biopython.motifs contains the core Motif class containing various I/O methods
...
def parse(handle, fmt, strict=True):
    ...
    it is easier to use biopython.motifs.read() instead of biopython.motifs.parse()
    for those.
    ...
    fmt = fmt.lower()
    if fmt == "alignace":
        from biopython.motifs import alignace

        return alignace.read(handle)
    elif fmt == "meme":
        from biopython.motifs import meme

        return meme.read(handle)
    elif fmt == "minimal":
        from biopython.motifs import minimal

        return minimal.read(handle)
    elif fmt == "clusterbuster":
        from biopython.motifs import clusterbuster

        return clusterbuster.read(handle)
    elif fmt in ("pfm-four-columns", "pfm-four-rows"):
        from biopython.motifs import pfm

        return pfm.read(handle, fmt)
    elif fmt == "xms":
        from biopython.motifs import xms

        return xms.read(handle)
    elif fmt == "mast":
        from biopython.motifs import mast

        return mast.read(handle)
    elif fmt == "transfac":
        from biopython.motifs import transfac

        return transfac.read(handle, strict)
    elif fmt in ("pfm", "sites", "jaspar"):
        from biopython.motifs import jaspar
...
def read(handle, fmt, strict=True):
    ...
    This supports the same formats as biopython.motifs.parse(), but
    ...
    Use the biopython.motifs.parse(handle, fmt) function if you want
