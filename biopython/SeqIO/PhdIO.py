...
# package.
"""biopython.SeqIO support for the "phd" file format.

PHD files are output by PHRED and used by PHRAP and CONSED.

You are expected to use this module via the biopython.SeqIO functions, under the
format name "phd". See also the underlying biopython.Sequencing.Phd module.

For example, using biopython.SeqIO we can read in one of the example PHRED files
...
QUAL files, for example using biopython.SeqIO.write(...), or simply with the format
...
def PhdIterator(source):
    ...
    This uses the biopython.Sequencing.Phd module to do the hard work.
    """
