...
# package.
"""biopython.SeqIO support for the UCSC nib file format.
...
You are expected to use this module via the biopython.SeqIO functions under
the format name "nib":

    >>> from biopython import SeqIO
    >>> record = SeqIO.read("Nib/test_even_bigendian.nib", "nib")
    >>> print("%i %s..." % (len(record), record[:20]))
...
class NibIterator(SequenceIterator):
    ...
    def __init__(self, source):
        """Iterate over a nib file and yield a Seq object.
        ...
        The sequence of the resulting Seq object should match the sequence
...
class NibWriter(SequenceWriter):
    ...
    def write_record(self, sequence):
        """Write a single sequence to the output file."""
        handle = self.handle
