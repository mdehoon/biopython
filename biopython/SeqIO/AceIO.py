...
# package.
"""biopython.SeqIO support for the "ace" file format.

You are expected to use this module via the biopython.SeqIO functions.
See also the biopython.Sequencing.Ace module which offers more than just accessing
the contig consensus sequences in an ACE file as Seq objects.
"""
...
def AceIterator(source):
    """Return Seq objects from an ACE file.

    This uses the biopython.Sequencing.Ace module to do the hard work.  Note that
    ...
    using PHRED scores using biopython.SeqIO, these are stored in the Seq object's
    ...
    ...         print("%s %s... %i" % (record.id, record[:10], len(record)))
    ...
    ...     for record in SeqIO.parse(handle, "ace"):
    ...         print("%s ...%s..." % (record.id, record[85:95]))
    ...
    for ace_contig in Ace.parse(source):
        # Convert the ACE contig record into a Seq object...
        consensus_seq = ace_contig.sequence
        if "*" in consensus_seq:
            # For consistency with most other file formats, map
            # any * gaps into - gaps.
            assert "-" not in consensus_seq
        ...
        seq_record = Seq(consensus_seq, id=ace_contig.name, name=ace_contig.name)
        ...
        seq_record.letter_annotations["phred_quality"] = tuple(quals)
