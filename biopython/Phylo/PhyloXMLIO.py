...
file from a ``biopython.Phylo.PhyloXML`` object.

About capitalization:
 - phyloXML means the file format specification
 - PhyloXML means the biopythonpython module ``biopython.Phylo.PhyloXML`` and its classes
 - Phyloxml means the top-level class used by ``PhyloXMLIO.read`` (but not
   ``biopython.Phylo.read``!), containing a list of Phylogenies (objects derived from
   ``BaseTree.Tree``)
...
from biopython.Phylo import PhyloXML as PX
...
def read(file):
    ...
    :returns: a single ``biopython.Phylo.PhyloXML.Phyloxml`` object.
...
def parse(file):
    ...
    :returns: a generator of ``biopython.Phylo.PhyloXML.Phylogeny`` objects.
