...
"""Base classes for biopython.Phylo objects.
...
def _combine_args(first, *rest):
    ...
    # same method in biopython.Nexus.Trees); root_with_outgroup and common_ancestor
...
class TreeElement:
    """Base class for all biopython.Phylo classes."""
...
class TreeMixin:
    ...
    def find_elements(self, target=None, terminal=None, order="preorder", **kwargs):
        ...
        :Parameters:
            target : TreeElement instance, type, dict, or callable
                ...
                TreeElement, matches any standard biopython.Phylo type.)
            terminal : bool
    ...
    def is_monophyletic(self, terminals, *more_terminals):
        ...
        To match both ``biopython.Nexus.Trees`` and the other multi-target methods in
        biopython.Phylo, arguments to this method can be specified either of two ways:
...
class Tree(TreeElement, TreeMixin):
    ...
    def as_phyloxml(self, **kwargs):
        ...
        """
        from biopython.Phylo.PhyloXML import Phylogeny
    ...
    def __format__(self, format_spec):
        ...
        :param format_spec: a lower-case string supported by ``biopython.Phylo.write``
            as an output file format.

        """
        if format_spec:
            from io import StringIO
            from biopython.Phylo import _io
