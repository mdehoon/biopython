...
The functionality is somewhat similar to Biopython's biopython.Entrez module which
...
def entry(db, id, format=None, field=None):
    ...
    EFetch, available in biopythonpython as biopython.Entrez.efetch(...), but that
...
def search_count(db, query):
    ...
    batches using the offset and limit options to biopython.TogoWS.search(). In
    general however the biopython.TogoWS.search_iter() function is simpler to use.
    """
...
def search_iter(db, query, limit=None, batch=100):
    ...
    Internally this first calls the biopython.TogoWS.search_count() and then
    uses biopython.TogoWS.search() to get the results in batches.
    """
...
def search(db, query, offset=None, limit=None, format=None):
    ...
    available in Biopython as the biopython.Entrez.esearch() function.

    See also the function biopython.TogoWS.search_count() which returns the number
    of matches found, and the biopython.TogoWS.search_iter() function which allows
...
def convert(data, in_format, out_format):
    ...
    alignnent file formats (functions biopython.SeqIO.convert and biopython.AlignIO.convert)
    """
