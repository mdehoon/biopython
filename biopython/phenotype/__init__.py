...
Input
=====
The main function is biopython.phenotype.parse(...) which takes an input file,
...
Output
======
Use the function biopython.phenotype.write(...), which takes a complete set of
...
Note that while biopython.phenotype can read the above file formats, it can only
...
warnings.warn(
    "biopython.phenotype is an experimental submodule which may undergo "
...
def parse(handle, format):
    ...
    Use the biopython.phenotype.read(...) function when you expect a single record
    only.
    """
    # Try and give helpful error messages:
    if not isinstance(format, str):
...
def read(handle, format):
    ...
    Use the biopython.phenotype.parse(handle, format) function if you want
