...
Input
=====
The main function for parsing search output files is biopython.SearchIO.parse(...).
...
SearchIO also provides the biopython.SearchIO.read(...) function, which is intended
...
functions biopython.SearchIO.index(...) or biopython.SearchIO.index_db(...). They have a
...
biopython.SearchIO.write(...) function. This function returns a tuple of four
...
Conversion
==========
SearchIO provides a shortcut function biopython.SearchIO.convert(...) to convert a
...
Note that the same restrictions found in biopython.SearchIO.write(...) applies to the
...
written using biopython.SearchIO.write(...).
...
from biopython.SearchIO._utils import get_processor
...
def index_db(index_filename, filenames=None, format=None, key_function=None, **kwargs):
    ...
    the biopython.SearchIO.index(...) function instead would use less memory.

    BGZF compressed files are supported, and detected automatically. Ordinary
    GZIP compressed files are not supported.

    See also biopython.SearchIO.index(), biopython.SearchIO.to_dict(), and the Python module
