...
``biopython.Entrez`` all have a ``.url`` attribute instead giving the URL
...
``biopython.Entrez.max_tries`` and ``biopython.Entrez.sleep_between_tries``.
...
def read(handle, validate=True, escape=False):
    ...
    to the correct Unicode string. The functions in biopython.Entrez to access NCBI
