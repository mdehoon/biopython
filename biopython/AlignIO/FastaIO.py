...
# package.
"""biopython.AlignIO support for "fasta-m10" output from Bill Pearson's FASTA tools.
...
Pearson's FASTA tools, for use from the biopython.AlignIO interface where it is
...
developed as an input format to the FASTA tools.  The biopython.AlignIO and
biopython.SeqIO both use the biopython.SeqIO.FastaIO module to deal with these files,
...
def FastaM10Iterator(handle, seq_count=None):
    ...
    This class is intended to be used via the biopython.AlignIO.parse() function
...
"""
        ...
        # See also biopython/AlignIO/MafIO.py for same requirement.
