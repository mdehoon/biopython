...
# package.
"""biopython.AlignIO support for the "nexus" file format.

You are expected to use this module via the biopython.AlignIO functions (or the
biopython.SeqIO functions if you want to work directly with the gapped sequences).

See also the biopython.Nexus module (which this code calls internally),
as this offers more than just accessing the alignment or its
sequences as Seq objects.
"""


from biopython.Nexus import Nexus
from biopython.Align import MultipleSeqAlignment
from biopython.AlignIO.Interfaces import AlignmentWriter
...
def NexusIterator(handle, seq_count=None):
    ...
    Thus uses the biopython.Nexus module to do the hard work.

    You are expected to call this function via biopython.SeqIO or biopython.AlignIO
    ...
    # biopython.Nexus deals with duplicated names by adding a '.copy' suffix.
...
class NexusWriter(AlignmentWriter):
    ...
    You are expected to call this class via the biopython.AlignIO.write() or
    biopython.SeqIO.write() functions.
    """
