...
# package.
"""biopython.AlignIO support for the "maf" multiple alignment format.
...
class MafWriter(SequentialAlignmentWriter):
    ...
    def _write_record(self, record):
        """Write a single Seq object to an 's' line in a MAF block (PRIVATE)."""
        ...
        fields = [
            ...
            "%5s"
            % record.annotations.get("size", len(str(record).replace("-", ""))),
            strand,
    ...
    def write_alignment(self, alignment):
        ...
        Writes every Seq object in a MultipleSeqAlignment object to its own
...
def MafIterator(handle, seq_count=None):
    ...
    MultipleSeqAlignment objects. Seq IDs generally correspond to species
    names.
    """
    in_a_bundle = False
    ...
    while True:
        ...
        if in_a_bundle:
            if line.startswith("s"):
                # add a Seq object to the bundle
                ...
                # Can then store in each Seq object's .letter_annotations dictionary,
                ...
                # See also biopython/AlignIO/FastaIO.py for same requirement.
        ...
        *ends* should be the list of the corresponding segment ends
        ...
        if len(fetched) == 0:
            return MultipleSeqAlignment(
                [Seq("N" * expected_letters, id=self._target_seqname)]
            )
        ...
        for multiseq in fetched:
            ...
            for gapped_pos in range(0, rec_length):
                for seqrec in multiseq:
                    ...
                    # in front of the "-"s
                    split_by_position[seqrec.id][real_pos] += seqrec[gapped_pos]
        ...
        for seqid, seq in subseq.items():
            ...
            result_multiseq.append(Seq(seq, id=seqid, name=seqid, description=""))
