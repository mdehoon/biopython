...
"""biopython.AlignIO support for "stockholm" format (used in the PFAM database).
...
using the biopython.AlignIO.read() function:
...
using biopython.AlignIO.write(), or the MultipleSeqAlignment object's format method:
...
Finally, as an aside, it can sometimes be useful to use biopython.SeqIO.parse() to
