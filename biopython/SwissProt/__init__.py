...
from biopython.SeqFeature import (
    SeqFeature,
    FeatureLocation,
    ExactPosition,
    BeforePosition,
...
class FeatureTable(SeqFeature):
    ...
    This is a subclass of SeqFeature, defined in biopython.SeqFeature, where the
    attributes are used as follows:
     ...
     - ``location``: location of the feature on the canonical or isoform
       ...
       defined in biopython.SeqFeature, with the ref attribute set to the isoform
...
def parse(source):
    ...
    Returns a generator object which yields biopython.SwissProt.Record() objects.
    """
    handle = _open(source)
    try:
        while True:
